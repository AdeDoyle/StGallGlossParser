...
def clean_analysis(taglist):
            ...
            if An3 in verb_tensepers:
                if actpas == 'Active':
                    if not rel:
                        ...
                        pos = "VERB"
    # General Verbs
        elif An2 in ['AI', 'AII', 'AIII', 'BI', 'BII', 'unclear', 'inflexion not clear']:
            if An3 in verb_tensepers:
                if actpas == 'Active':
                    if not rel:
                        pos = "VERB"
        ...
        if An2 in ['negative', 'prefix, negative']:
            if not An3:
                if not actpas:
                    if not rel:
                        pos = "PART"
    # Assign Interrogative Particles (PART)
    ...
    if An1 == 'number':
        if An2 == 'adjective':
            if An3 in ['uninflected', 'nom.du.fem.', 'acc.du.fem.', 'composition form']:
                if not actpas:
                    if not rel:
                        pos = "NUM"
